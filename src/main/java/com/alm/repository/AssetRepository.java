...
/**
 * 자산 도메인 Repository.
 * Class Table Inheritance 구조: asset_master + 4개 자식 테이블(ACC/REA/PHY/CSH).
 * 삭제는 asset_master 만 하면 자식 테이블은 ON DELETE CASCADE 로 자동 삭제된다.
 */
public class AssetRepository {
    ...
    /**
     * 전체 자산 목록 반환. 4개 타입을 각각 조회 후 합산한다.
     * ACC 는 bank_id / type_id 가 NULL 인 계좌도 포함하기 위해 LEFT JOIN 사용.
     */
    ...
    /** asset_master 삭제. 자식 테이블은 ON DELETE CASCADE 로 자동 삭제. */
    ...
    /**
     * 은행 목록 반환.
     * 주의: InvestRepository 에서 type_name = '증권위탁계좌' 로 조회하므로 시드 데이터와 일치해야 한다.
     */
